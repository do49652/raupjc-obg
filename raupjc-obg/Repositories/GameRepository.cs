...
namespace raupjc_obg.Repositories
{
    public class GameRepository : IGameRepository
    {
        ...
        {
            return await _dbContext.Games.Where(g => !g.Private).ToListAsync();
        }
        ...
        {
            return await _dbContext.Games.Where(g => g.UserId.Equals(userId)).ToListAsync();
        }

        public async Task<List<GameModel>> GetPublicGames(Guid userId)
        {
            return await _dbContext.Games.Where(g => g.UserId.Equals(userId) && !g.Private).ToListAsync();
        }
        ...
        {
            return await _dbContext.Games.FirstOrDefaultAsync(g => g.Id.Equals(id));
        }

        public async Task<GameModel> GetGameByName(string name)
        {
            return await _dbContext.Games.FirstOrDefaultAsync(g => g.Name.Equals(name));
        }
