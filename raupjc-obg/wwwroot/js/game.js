        ...
        if (!gameStarted) {
            if (message == "admin") {
                document.getElementById("startgame").innerHTML =
                    "<button class='btn btn-primary' onclick=\"ws.send('start');\">Start game</button>";
        ...
        if (message == "ready") {
            ...
            $(function() {
                ...
                for (let i = 0; i < game["Log"].length; i++)
                    log += '<span data-toggle="tooltip" data-placement="left auto" data-container="body" title="' +
                        game["Log"][i].split("]")[0].substring(1) +
                        '">' +
                        game["Log"][i].replace(/\[([a-z0-9_ :-]*)\]/i, "") +
                        "</span><br>";
                $("#log").text("").append(log);
                ...
                for (let i = 0; i < Object.keys(game["Players"]).length; i++)
                    log += Object.keys(game["Players"])
                        [i] +
                        ": " +
                        game["Players"][Object.keys(game["Players"])[i]]["Space"] +
                        "\n";
                ...
                for (let i = 0; i < game["Players"][username]["Items"].length; i++)
                    log += '<button class="btn btn-default" id="item' +
                        (i + 1) +
                        '">' +
                        game["Players"][username]["Items"][i]["Name"] +
                        "</button>";
            ...
            var sceneManager = function() {
                if (game["Scene"] == "roll") {
                    ...
                    $(function() {
                        ...
                        for (let i = 0;
                            i < Object.keys(game["Players"][username]["CurrentEvent"]["Items"]).length;
                            i++) {
                            items.push({
                                name: Object.keys(game["Players"][username]["CurrentEvent"]["Items"])[i],
                                description: game["Players"][username]["CurrentEvent"]["Items"][Object.keys(
                                    game["Players"][username]["CurrentEvent"]["Items"])[i]][0]["Description"],
                                price: parseFloat(
                                    game["Players"][username]["CurrentEvent"]["Items"][Object.keys(
                                        game["Players"][username]["CurrentEvent"]["Items"])[i]][1])
                            });
                        }
                        ...
                        for (i in items) {
                            ...
                            $("#proceed" + (i + 1)).text("")
                                .append(items[i].name + " <i>(Cost: " + items[i].price + ")</i>");
                            if (playingUsername == username &&
